...
const SearchProjects = () => {
    ...
    const filterProjects = searchProject === '' ? [] : projects.filter(project => project.name.toLowerCase().startsWith(searchProject.toLowerCase().trim()))
    console.log(projects)
    console.log(filterProjects)
    return (
        <Transition.Root show={search} as={Fragment}
            afterLeave={() => setSearchProject('')}
        >
            <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={handleSearch}>
                ...
                >
                    <Combobox
                        ...
                        onChange={ (project) => (window.location.href = `/projects/${project}`) }
                    >
